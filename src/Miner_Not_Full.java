...
public class Miner_Not_Full extends AbstractMiner{
    ...
    public Ghost transformGhost(WorldModel world, EventScheduler scheduler, ImageStore imageStore){
        Ghost ghost = this.getPosition().createGhost(this.getId(), this.getPosition(), imageStore.getImageList(WorldModel.GHOST_KEY), this.getActionPeriod(), this.getAnimationPeriod());
        world.removeEntity(this);
        scheduler.unscheduleAllEvents(this);
